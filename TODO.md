...
| `Open` | `High` | Import fidelity | Restore plugin analytics and non-derivable solve metadata on project import | Imported projects currently lose plugin analytics and CO2 shadow information. | 10,000 | Partially closed by PR #11 (full `RunResults` derived locally); remaining gap is `co2Shadow` (needs constraint duals from the linopy model) and `pluginAnalytics` (no plugin re-execution on import). |
...
| `Open` | `High` | Analytics | Add multi-period and scenario-aware result analytics | Pathway/stochastic support requires different result views, comparisons, and summaries. | 20,000 | Period KPI strip, period summary table, and capacity-over-periods chart are in (PR #15, #16). Scenario-aware analytics (comparing scenarios side by side) is still open. |
